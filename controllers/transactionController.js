...
const transaction_post = async (req, res) => {
  ...
  const schema = z.object({
    title: z.string().min(2, { message: "Title must be at least two characters" }),
    money: z.number().positive({ message: "Money must be a positive number" }),
...
const transaction_delete = async (req, res) => {
  ...
  try {
    deletedTransaction = await prisma.transaction.deleteMany({
      where: {
        id: transactionId,
        userId: req.session.userId,
      },
